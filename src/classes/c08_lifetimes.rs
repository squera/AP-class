...
// - If lifetime `'b` is longer than `'a`, we can use `'b` where `'a` is expected;
//      essentially performing lifetime subtyping.
//      We can require this with `'a: 'b`.
//      which reads "  'a that outlives 'b  "
// - Permits us to call `longest(&x,&y)` when `x` and `y` have different lifetimes,
//      but one outlives the other.
