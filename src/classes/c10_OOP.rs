...
//  > The procedures are typically called methods or
//  > operations.[name=The Gang of Four book]
//
...
pub struct AveragedCollection {
    ...
    // there’s no way for external code to add or remove
    // items to the list field directly,
...
// `AveragedCollection`, we can easily change aspects like
// the data structure in the future.
// For instance, we could use a `HashMap` instead of a `Vec`
// for the list field.
// As long as the signatures of the `add`, `remove`,
// and `average` public methods stay the same,
// code using `AveragedCollection` wouldn’t need to change.
// If we made `list` public instead, this wouldn’t
// necessarily be the case:
// `HashMap` and `Vec` have different methods for adding
// and removing items,
// so the external code would likely have to change if
// it was modifying list directly.
//
// If encapsulation is a required aspect for a language
// to be considered object-oriented,
// then Rust meets that requirement.
// The option to use `pub` or not for different parts
// of code enables encapsulation of implementation details.
...
/*  Inheritance is a mechanism whereby an object can inherit
    from another object’s definition, thus gaining the
    parent object’s
    ...
    Why does one want Inheritance to begin with, though?
        A) code reuse
        B) subtyping polymorphism: using an object
            of type A where one of type B is expected,
            if A extends B

    Rust provides some features for both
        A and B

    A) For A, we have Generics and Traits,
...
// Dynamic Dispatch to Rust, which is possible with
//      Trait Objects
// which, technically, is an object

// A trait object `&dyn Trait` (for some Trait) points to both
//      an instance of a **pointer type** implementing
//          our specified trait
//      as well as a table used to look up trait methods
//          on that type at runtime.

// Example: using Trait objects in Vectors.
// Recall: the Rust compiler restricts that all the
//  values in a vector **must** have the same type.
// Even if we define a generic type, the generic type
//  can be substituted with **one** concrete type at a time.
...
// A **Trait object** tells rust to pass all types
//  that implement the trait.
// so we can declare a Trait: Show and use it
// to populate a vector of Show s
trait Show {
    fn show(&self) -> String;
}
...
// Rust follows this orphan rule: if you want to
// implement a trait for your data type,
// at least one of them must be local.
...
pub fn example_animals_oop() {
    ...
    // now some subtleties, these 2 pairs of functions
...
// and the quack_ref.
// The body of quack_trait is compiled for each of
// the calling types and no virtual method is needed;
...
// Now, if I have a value foo of type Foo, then a reference
// to that value will satisfy
